        ...
        try {
            for (const module of await promises.readdir(module_path, { withFileTypes: true })) {
                try {
                    ...
                    const moduleParts = await promises.readdir((0, path_1.join)(module_path, module.name, srcPart));
                    for (const part of moduleParts.map(p => p.toLowerCase())) {
                        if (part === 'commands') {
                            commands.push(...(await loadCommands((0, path_1.join)(module_path, module.name, srcPart, part), module.name, logger)));
                            await logger.logToFile('default', `- loaded commands of ${module.name}`);
                        }
                        else if (part === 'events') {
                            events.push(...(await loadEvents((0, path_1.join)(module_path, module.name, srcPart, part), logger)));
                            await logger.logToFile('default', `- loaded events of ${module.name}`);
                        }
                        else if (part === 'entities') {
                            entities.push((0, path_1.join)(module_path, module.name, srcPart, part, '*.js'));
