...
const MODULE_PATHS = [
    (0, path_1.join)(__dirname, '..', 'modules'),
    (0, path_1.join)('.', 'modules')
];
    ...
    for (const module_path of MODULE_PATHS) {
        try {
            for (const module of await promises.readdir(module_path)) {
                ...
                try {
                    const moduleParts = await promises.readdir((0, path_1.join)(module_path, module, 'src'));
                    for (const part of moduleParts) {
                        if (part === 'commands') {
                            commands.push(...(await loadCommands((0, path_1.join)(module_path, module, 'src', part), module, logger)));
                        }
                        else if (part === 'events') {
                            events.push(...(await loadEvents((0, path_1.join)(module_path, module, 'src', part), logger)));
                        }
                        else if (part === 'entities') {
                            entities.push((0, path_1.join)(module_path, module, 'src', part, '*.js'));
                        }
                    }
                }
                catch (e) {
