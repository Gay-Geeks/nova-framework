...
async function client(settings) {
    ...
    await logger.logToFile('debug', `Client created with intents: ${Array.isArray(clientOptions.intents) ? clientOptions.intents.join(', ') : JSON.stringify(clientOptions.intents)}`);
    ...
    bot.events.on('reloadCommands', () => void (0, commandHandler_1.uploadCommands)(settings.token, client, bot).catch((error) => logger.logError(error)));
